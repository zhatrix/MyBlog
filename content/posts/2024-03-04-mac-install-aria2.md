...
```bash
aria2c --conf-path="/Users/username/.aria2/aria2.conf"
# username 替换为你的Mac用户名
```
...
```bash
03/04 17:59:16 [NOTICE] IPv4 RPC: listening on TCP port 6800
03/04 17:59:16 [NOTICE] IPv6 RPC: listening on TCP port 6800
```

把每次启动的内容写入脚本，简化启动。

由于每次打开终端，默认地址是 登录用户的地址“/Users/username”，所以在该目录创建一个xx.sh的脚本。
...
使用本地客户端AriaNG Native连接服务器和本地的Aria2进程

[https://github.com/mayswind/AriaNg-Native/releases](https://github.com/mayswind/AriaNg-Native/releases)

下载对应的Mac版AriaNG Native

打开App之后，在系统设置-AriaNg设置-RPC本地，找到Aria2 RPC 密钥*，设置密钥，然后保存刷新即可。
...
[https://www.mintimate.cn/2019/06/21/Aria2/#连接Aria2进程](https://www.mintimate.cn/2019/06/21/Aria2/#连接Aria2进程)
